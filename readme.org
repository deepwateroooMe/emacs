...
* TODOs: tracing bugs of emacs 28.1 custom-build version
** invalid time specification: sr-speedbar on MacOS
   
[[./pic/readme_20230208_163916.png]]
...
- 今天又尝试安装Xcode之后再构建一遍，但是没有成功.可能本身参考有些年代lp，另外自己还完全不通这个部分，所以暂时放一放.改天有机会可以再回来研究一下，错在哪里，我如何才可能构建出自己的版本. 
...
- added key-bindings for opening from VSCode/Android Studio of current emacs buffer. 
  - VSC Emacs can locate to each other to correct row and col. 
