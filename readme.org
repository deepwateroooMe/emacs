...
- 纪录原 28.2 版本里的、存在的、不便利、【BUG：】及其它：
  - csharp-mode 环形引用？语法不高亮？简单解决了，暂时没有、没检测到、有任何问题
  - 【TODO】：想要测试 *sr-speedbar 里，Timer 的【BUG：】仍然存在* ；先前存在的、自动目录更新问题，2 个 bug 都存在。这个， *明天或后天，一两天之内，把它解决掉。但希望亲爱的表哥的活宝妹，这次能够解决得，更为彻底一点儿。希望实现先前 emacs 版本里存在过的、自动目录更新等便利功能。*
